<template>
  ...
  <div class="org-description-editor">
    <!-- Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-2 mb-2 p-2 bg-gray-50 rounded-t-lg border border-b-0 border-gray-200">
      <div class="flex flex-wrap items-center gap-2">
        <template v-if="!showPreview">
          ...
          <button type="button" @click="insertLink()" title="Lien" class="toolbar-btn flex items-center gap-1">
          ...
          <div class="relative" ref="imageDropdownRef">
            ...
            <div v-if="showImagePanel"
              ...
              <div v-if="!organizationId" class="p-3 text-xs text-gray-500">
                Sélectionnez une organisation pour ajouter des images.
              </div>
              <template v-else>
                <!-- Upload new image -->
        ...
        <span v-else class="text-xs font-medium text-gray-500 px-1">
          Mode aperçu
        </span>
      </div>
      ...
      <button type="button" @click="showPreview = !showPreview"
        class="toolbar-btn flex items-center gap-1.5 font-medium transition-colors"
        :class="{ 'bg-indigo-50 border-indigo-300 text-indigo-700': showPreview }"
        :title="showPreview ? 'Retourner à l\'édition' : 'Voir l\'aperçu'">
        <component :is="showPreview ? PencilSquareIcon : EyeIcon" class="h-3.5 w-3.5" />
        <span class="text-xs">{{ showPreview ? 'Éditer' : 'Aperçu' }}</span>
      </button>
    </div>

    <!-- Preview area -->
    <div v-if="showPreview"
      class="block w-full min-h-[12rem] rounded-b-lg border border-gray-200 bg-white px-4 py-3 sm:text-sm">
      <OrgDescriptionRenderer
        v-if="modelValue && modelValue.trim()"
        :description="modelValue"
        :members="members"
        :allowed-image-urls="images.map(i => i.url).filter((u): u is string => u !== null)"
      />
      <p v-else class="text-xs text-gray-400 italic">Aucune description à afficher.</p>
    </div>

    <!-- Textarea -->
    <textarea v-else ref="textareaRef" :value="modelValue" @input="$emit('update:modelValue', ($event.target as HTMLTextAreaElement).value)" :rows="rows"
    ...
    <p class="mt-1 text-xs text-gray-400">
      Syntaxe : <code>**gras**</code>, <code>*italique*</code>, <code># titre</code>, <code>[alt](url)</code>, <code>@[Nom Membre]</code> ou <code>@membre</code>,
      <code>![alt](url)</code>
    </p>
  </div>
  </DocsHint>
...
import { UserCircleIcon, PhotoIcon, ArrowUpTrayIcon, XMarkIcon, LinkIcon, EyeIcon, PencilSquareIcon } from '@heroicons/vue/24/outline'
...
const props = defineProps({
  ...
  placeholder: {
    type: String,
    default: "Écrivez une description... Utilisez la barre d'outils pour insérer des membres ou des images."
  },
...
const toggleImagePanel = async () => {
  ...
  if (showImagePanel.value && props.organizationId) {
    await loadImages()
  }
}
...
const uploadImage = async (event: Event) => {
  ...
  if (!file) return
  if (!props.organizationId) {
    uploadError.value = "Veuillez sélectionner une organisation pour ajouter une image."
    return
  }
...
function insertLink() {
  const { selected } = getSelection()
  const text = selected || 'alt'
  replaceSelection(`[${text}](https://)`, text.length + 11)
}
