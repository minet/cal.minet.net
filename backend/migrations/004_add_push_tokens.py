...
def run_migration():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if not result:
                logger.info("Adding notification_delay column to user table...")
                alter_sql = text(
                    'ALTER TABLE "user" ADD COLUMN notification_delay INTEGER DEFAULT 15'
                )
                conn.execute(alter_sql)
                logger.info("Migration successful.")
            else:
                logger.info("Column notification_delay already exists.")
