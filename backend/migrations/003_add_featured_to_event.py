...
def run_migration():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if not result:
                logger.info("Adding featured column to event table...")
                alter_sql = text(
                    "ALTER TABLE event ADD COLUMN featured INTEGER DEFAULT 0 NOT NULL"
                )
                conn.execute(alter_sql)
                logger.info("Migration successful.")
            else:
                logger.info("Column featured already exists.")
